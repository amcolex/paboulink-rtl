...
// Generated on Sat Nov 01 00:12:42 UTC 2025
...
// latency=980, gap=1024
   rc62087 stage0(.clk(clk), .reset(reset), .next(next_0), .next_out(next_1),
    .X0(t0_0), .Y0(t1_0),
    .X1(t0_1), .Y1(t1_1),
    .X2(t0_2), .Y2(t1_2),
    .X3(t0_3), .Y3(t1_3));
...
// latency=11287, gap=11287
   ICompose_66412 IComposeInst66623(.next(next_1), .clk(clk), .reset(reset), .next_out(next_2),
       .X0(t1_0), .Y0(t2_0),
       .X1(t1_1), .Y1(t2_1),
       .X2(t1_2), .Y2(t2_2),
       .X3(t1_3), .Y3(t2_3));
...
// Latency: 980
// Gap: 1024
module rc62087(clk, reset, next, next_out,
   X0, Y0,
   X1, Y1,
   X2, Y2,
   X3, Y3);
   ...
   perm62085 instPerm66624(.x0(t0), .y0(s0),
    .x1(t1), .y1(s1),
...
// Latency: 980
// Gap: 1024
module perm62085(clk, next, reset, next_out,
   x0, y0,
   x1, y1);
   ...
   nextReg #(978, 10) nextReg_66629(.X(next), .Y(next0), .reset(reset), .clk(clk));


   shiftRegFIFO #(2, 1) shiftFIFO_66632(.X(next0), .Y(next_out), .clk(clk));


   memArray2048_62085 #(numBanks, logBanks, depth, logDepth, width)
...
module memArray2048_62085(next, reset,
                x0, y0,
                inAddr0,
                outAddr0,
                x1, y1,
   ...
   nextReg #(1024, 10) nextReg_66637(.X(next), .Y(next0), .reset(reset), .clk(clk));
...
// Latency: 11287
// Gap: 11287
module ICompose_66412(clk, reset, next, next_out,
      X0, Y0,
      X1, Y1,
      X2, Y2,
      X3, Y3);
   ...
   statementList66410 instList66642 (.clk(clk), .reset(reset), .next(int_next), .next_out(int_next_out),
      .i1_in(i1),
    .X0(s0), .Y0(t0),
    .X1(s1), .Y1(t1),
    .X2(s2), .Y2(t2),
...
// Latency: 1025
// Gap: 1024
// module_name_is:statementList66410
module statementList66410(clk, reset, next, next_out,
   i1_in,
   X0, Y0,
   X1, Y1,
   X2, Y2,
...
// latency=11, gap=1024
   DirSum_66325 DirSumInst66645(.next(next_0), .clk(clk), .reset(reset), .next_out(next_1),
.i1(i1_0),
       .X0(t0_0), .Y0(t1_0),
       .X1(t0_1), .Y1(t1_1),
       .X2(t0_2), .Y2(t1_2),
...
// latency=2, gap=1024
   codeBlock66327 codeBlockIsnt66646(.clk(clk), .reset(reset), .next_in(next_1), .next_out(next_2),
       .X0_in(t1_0), .Y0(t2_0),
       .X1_in(t1_1), .Y1(t2_1),
       .X2_in(t1_2), .Y2(t2_2),
       .X3_in(t1_3), .Y3(t2_3));
...
// latency=1012, gap=1024
   rc66408 instrc66647(.clk(clk), .reset(reset), .next(next_2), .next_out(next_3),
    .X0(t2_0), .Y0(t3_0),
    .X1(t2_1), .Y1(t3_1),
    .X2(t2_2), .Y2(t3_2),
    .X3(t2_3), .Y3(t3_3));
...
// Latency: 11
// Gap: 1024
module DirSum_66325(clk, reset, next, next_out,
      i1,
      X0, Y0,
      X1, Y1,
      X2, Y2,
   ...
   codeBlock62089 codeBlockIsnt66648(.clk(clk), .reset(reset), .next_in(next), .next_out(next_out),
.i2_in(i2),
.i1_in(i1),
       .X0_in(X0), .Y0(Y0),
       .X1_in(X1), .Y1(Y1),
...
// Latency: 11
// Gap: 1
module codeBlock62089(clk, reset, next_in, next_out,
   i2_in,
   i1_in,
   X0_in, Y0,
   X1_in, Y1,
   ...
   shiftRegFIFO #(10, 1) shiftFIFO_66651(.X(next), .Y(next_out), .clk(clk));
   ...
   wire  [11:0] a63;
   reg signed [11:0] tm14;
   reg signed [11:0] tm21;
   reg signed [11:0] tm42;
   reg signed [11:0] tm52;
   reg signed [11:0] tm15;
   reg signed [11:0] tm22;
   reg signed [11:0] tm43;
   ...
   D1_64273 instD1inst0_64273(.addr(a63[10:0]), .out(tm5), .clk(clk));

   D2_66323 instD2inst0_66323(.addr(a63[10:0]), .out(tm6), .clk(clk));

    addfxp #(12, 1) add62108(.a({1'b0, a57}), .b({11'b0, tm1}), .clk(clk), .q(a58));    // 0
    subfxp #(12, 1) sub62140(.a({1'b0, a62}), .b({1'b0, tm4}), .clk(clk), .q(a63));    // 2
    multfix #(12, 2) m62162(.a(tm7), .b(tm18), .clk(clk), .q_sc(a65), .q_unsc(), .rst(reset));
    multfix #(12, 2) m62184(.a(tm8), .b(tm25), .clk(clk), .q_sc(a67), .q_unsc(), .rst(reset));
    multfix #(12, 2) m62202(.a(tm8), .b(tm18), .clk(clk), .q_sc(a68), .q_unsc(), .rst(reset));
    multfix #(12, 2) m62213(.a(tm7), .b(tm25), .clk(clk), .q_sc(a69), .q_unsc(), .rst(reset));
    subfxp #(12, 1) sub62191(.a(a65), .b(a67), .clk(clk), .q(Y2));    // 9
    addfxp #(12, 1) add62220(.a(a68), .b(a69), .clk(clk), .q(Y3));    // 9
...
// Latency: 2
// Gap: 1
module codeBlock66327(clk, reset, next_in, next_out,
   X0_in, Y0,
   X1_in, Y1,
   X2_in, Y2,
   X3_in, Y3);
   ...
   shiftRegFIFO #(1, 1) shiftFIFO_66654(.X(next), .Y(next_out), .clk(clk));
   ...
   assign Y3 = t24;
    ...
    addfxp #(12, 1) add66339(.a(a9), .b(a10), .clk(clk), .q(t21));    // 0
    addfxp #(12, 1) add66354(.a(a11), .b(a12), .clk(clk), .q(t22));    // 0
    subfxp #(12, 1) sub66369(.a(a9), .b(a10), .clk(clk), .q(t23));    // 0
    subfxp #(12, 1) sub66384(.a(a11), .b(a12), .clk(clk), .q(t24));    // 0
...
// Latency: 1012
// Gap: 1024
module rc66408(clk, reset, next, next_out,
   X0, Y0,
   X1, Y1,
   X2, Y2,
   X3, Y3);
   ...
   perm66406 instPerm66655(.x0(t0), .y0(s0),
    .x1(t1), .y1(s1),
   .clk(clk), .next(next), .next_out(next_out), .reset(reset)
);
...
// Latency: 1012
// Gap: 1024
module perm66406(clk, next, reset, next_out,
   x0, y0,
   x1, y1);
   parameter numBanks = 2;
   parameter logBanks = 1;
   ...
   nextReg #(1010, 10) nextReg_66660(.X(next), .Y(next0), .reset(reset), .clk(clk));


   shiftRegFIFO #(2, 1) shiftFIFO_66663(.X(next0), .Y(next_out), .clk(clk));


   memArray2048_66406 #(numBanks, logBanks, depth, logDepth, width)
     memSys(.inFlip(inFlip0), .outFlip(outFlip0), .next(next), .reset(reset),
        .x0(w_1_0[width+logDepth-1:logDepth]), .y0(ybuff0),
        .inAddr0(w_1_0[logDepth-1:0]),
        .outAddr0(u_1_0), 
...
module memArray2048_66406(next, reset,
                x0, y0,
                inAddr0,
                outAddr0,
                x1, y1,
   ...
   output [width-1:0]  y1;
   input [logDepth-1:0] inAddr1, outAddr1;
   nextReg #(1024, 10) nextReg_66668(.X(next), .Y(next0), .reset(reset), .clk(clk));
